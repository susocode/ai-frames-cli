...
interface ItemMeta { file: string; title: string; description: string; version: string; scope: string }
type InstallState = 'idle' | 'saving' | 'saved'

interface Props { type: string; title: React.ReactNode; subtitle: string }
...
export default function MarketplacePage({ type, title, subtitle }: Props) {
  ...
  useEffect(() => {
    setLoading(true)
    setSearch('')
    fetch(`/api/marketplace/${type}`).then(r => r.json()).then(d => {
  ...
  async function save() {
    ...
    await fetch(`/api/marketplace/${type}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ selected }) })
    ...
    <div className="page">
      ...
      {filtered.length === 0 ? (
        <div className="marketplace-empty">
          <p>{search ? t.marketplace_no_results : t.marketplace_empty.replace('{type}', type)}</p>
        </div>
      ) : (
        <div className="marketplace-grid">
          {filtered.map(item => {
            ...
            return (
              <div key={item.file} className={`template-card ${isSelected ? 'selected' : ''}`} onClick={() => toggle(item.file)}>
                ...
                <div className="template-card-body">
                  <div className="template-card-header">
                    ...
                    <span style={{ flex: 1 }} />
                    <button className="template-preview-btn-corner" onClick={e => { e.stopPropagation(); setFilePreview(item) }} title="View content">
                  ...
                  {item.description && <span className="template-card-desc">{item.description}</span>}
                </div>
              </div>
            )
          })}
      ...
      {filePreview && <FileContentModal type={type} file={filePreview.file} title={filePreview.title} version={filePreview.version} onClose={() => setFilePreview(null)} />}

      <div className={`marketplace-footer ${hasChanges ? 'visible' : ''}`}>
        <span className="marketplace-footer-count">{selected.length} selected</span>
        <button className={`btn-primary ${installState === 'saved' ? 'btn-saved' : ''}`} disabled={installState === 'saving' || !hasChanges} onClick={save}>
          {installState === 'saving' ? t.marketplace_saving : installState === 'saved' ? t.marketplace_saved : t.marketplace_save_sync}
        </button>
      </div>
    </div>
  )
