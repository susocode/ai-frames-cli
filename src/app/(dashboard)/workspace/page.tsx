...
interface WorkspaceData { workspace: string; base: DirEntry[]; assistants: Record<string, AssistantDirs>; custom: CustomDirEntry[] }
...
export default function WorkspacePage() {
  ...
  async function persistMappings(updated: DirMapping[]) {
    await fetch('/api/custom-dirs', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updated) })
  ...
  useEffect(() => {
    fetch('/api/repo-status').then(r => r.json()).then(d => setRepoStatus(d.empty ? 'empty' : 'ok')).catch(() => setRepoStatus('unknown'))
    loadDirs()
  }, [loadDirs])

  useEffect(() => {
    function onSync() {
      fetch('/api/repo-status').then(r => r.json()).then(d => setRepoStatus(d.empty ? 'empty' : 'ok')).catch(() => {})
      loadDirs()
    }
  ...
  function toggle(id: string) {
    if (selected.has(id) && assistantSelections[id] === true) {
      if (!window.confirm(t.assistant_has_selections)) return
    }
  ...
  async function saveChanges() {
    ...
    await fetch('/api/assistants', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updated) })
  ...
  return (
    <div className="page">
      ...
      {repoStatus === 'checking' && <p className="text-muted">{t.assistants_repo_checking}</p>}

      {repoStatus === 'empty' && (
        <div className="repo-empty-banner">
          <p>{t.assistants_repo_empty}</p>
          <button className={`btn-primary ${initState === 'done' ? 'btn-saved' : ''}`} disabled={initState === 'initializing'} onClick={initRepo}>
            {initState === 'initializing' ? t.assistants_repo_initializing : initState === 'done' ? t.assistants_repo_init_done : t.assistants_repo_init_button}
          </button>
        </div>
      )}
      ...
      {repoStatus !== 'empty' && repoStatus !== 'checking' && (loadingDirs ? (
        ...
        <>
          ...
          <div className="custom-dirs-section">
            <CustomMappings
              ...
              onRemove={async (i, localPath, exists) => {
                if (exists) {
                  await fetch('/api/custom-dirs/local', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ local_path: localPath }) })
                }
          ...
          <div className={`marketplace-footer ${hasChanges ? 'visible' : ''}`}>
            <span className="marketplace-footer-count">{selected.size} {t.assistants_title.toLowerCase()}</span>
            <button
              className={`btn-primary ${saveState === 'saved' ? 'btn-saved' : ''}`}
              disabled={saveState === 'saving' || !hasChanges}
              onClick={saveChanges}
            >
              {saveState === 'saving' ? t.assistant_toggling : saveState === 'saved' ? t.workspace_recreate_done : t.context_settings_save}
            </button>
          </div>
        </>
      ))}
