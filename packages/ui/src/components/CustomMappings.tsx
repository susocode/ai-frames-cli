...
interface Props {
  mappings: DirMapping[]
  savedEntries?: CustomEntry[]   // status from server (exists + synced)
  saving?: boolean
  assistantPrefix?: string       // e.g. '.claude', '.copilot' — scopes the default local path
...
export default function CustomMappings({ mappings, savedEntries = [], saving = false, assistantPrefix, onAdd, onRemove }: Props) {
  ...
  const isDuplicate = !!newRepo.trim() && mappings.some(
    ...
    m.local_path === (newLocal.trim() || defaultLocalPath(newRepo.trim()))
  )

  async function handleAdd() {
    if (!newRepo.trim() || isDuplicate) return
    const r = newRepo.trim()
    const l = newLocal.trim()
    setNewRepo('')
    setNewLocal('')
    await onAdd(r, l)
  }
  ...
  return (
    <div className="custom-mappings">
          ...
          <tbody>
            {savedEntries.map((d, i) => (
              <tr key={d.local_path}>
                ...
                <td style={{ textAlign: 'right' }}>
                  <button className="btn-danger custom-dirs-remove" onClick={() => handleRemove(i)}>
      ...
      <div className="custom-dirs-row">
        ...
        <input
          className="custom-dirs-input"
          placeholder={newRepo.trim() ? defaultLocalPath(newRepo.trim()) : t.workspace_custom_local_placeholder}
          value={newLocal}
          onChange={e => setNewLocal(e.target.value)}
        />
        <button
          className="btn-primary"
          disabled={!newRepo.trim() || saving || isDuplicate}
          onClick={handleAdd}
        >
