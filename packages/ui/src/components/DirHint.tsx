...
const TOOLTIP_W = 280
const TOOLTIP_H = 120 // estimated max height

export default function DirHint({ dirPath }: Props) {
  const { t } = useLang()
  ...
  return (
    <span className="dir-hint-wrap">
      ...
      {style && createPortal(
        <span className="dir-hint-tooltip" style={style}>{hint}</span>,
        document.body
      )}
    </span>
  )
