    ...
    <div className="ctx-selector" ref={wrapRef}>
      ...
      {active && (
        <>
          ...
          <button
            className={`ctx-icon-btn ${syncState === 'updated' ? 'ctx-synced' : ''}`}
            onClick={onSync}
            disabled={syncState === 'checking' || syncState === 'syncing'}
            title={syncTitle}
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={syncState === 'checking' || syncState === 'syncing' ? 'spin' : ''}>
      ...
      {open && createPortal(
        <div className="ctx-dropdown" style={menuStyle}>
          ...
          {contexts.map(c => (
            <button
              key={c.id}
              className={`ctx-dropdown-item ${c.id === active?.id ? 'active' : ''}`}
