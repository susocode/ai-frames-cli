...
export default function ContextSettingsModal({ context, onClose, onSaved }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    resources_repo: context.resources_repo,
    workspace: context.workspace,
    token: context.auth?.token ?? '',
    ssh_key: context.auth?.key_path ?? '',
  })
  ...
  async function save() {
    ...
    if (res.ok) {
      ...
      setSaveErrorCode(data.code ?? 'unknown-error')
      setSaveState('idle')
    }
  }
      ...
      <div className="modal">
        ...
        <div className="modal-body">
          ...
          <div className="modal-field">
            <label>ID</label>
            <input value={context.id} readOnly className="input-readonly" />
          </div>
          ...
          <div className="modal-field">
            ...
            {form.name !== context.name && (
              <p className="modal-warning">
                ...
                {t.context_settings_name_warning}
              </p>
            )}
          </div>
          ...
          <div className="modal-field">
            ...
            {form.workspace !== context.workspace && isAbsolutePath(form.workspace) && (
              <p className="modal-warning">
                <span className="modal-warning-icon">⚠</span>
                {t.context_settings_repo_warning}
              </p>
            )}
          </div>
          ...
          <div className="modal-field">
            ...
            {form.resources_repo !== context.resources_repo && (
              <p className="modal-warning">
                <span className="modal-warning-icon">⚠</span>
                {t.context_settings_resources_warning}
              </p>
            )}
          </div>
          ...
          <div className="modal-provider-group">
            <div className="modal-field">
              ...
              {form.provider !== context.provider && (
                <p className="modal-warning">
                  <span className="modal-warning-icon">⚠</span>
                  {t.context_settings_provider_warning}
                </p>
              )}
            </div>
        ...
        <div className="modal-footer">
          {saveErrorCode && (
            <p className="modal-footer-error">✕ {errorMessage(t, saveErrorCode)}</p>
          )}
          <button className="btn-secondary" onClick={onClose}>
            {t.context_settings_cancel}
          </button>
          <button
          ...
          >
            {saveState === 'saving'
              ? t.context_settings_saving
              : saveState === 'saved'
              ? t.context_settings_saved
