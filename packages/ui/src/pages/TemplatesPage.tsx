...
export default function TemplatesPage() {
  ...
  function resourceSummary(tmpl: TemplateMeta): string {
    ...
    return Object.entries(tmpl.resources)
    ...
    <div className="page">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="templates-grid">
          {filtered.map(tmpl => {
            ...
            const isInstalled = installed.includes(tmpl.file)
                ...
                <div className="template-card-body">
                  <div className="template-card-header">
                    ...
                    <button
                      ...
                      onClick={e => { e.stopPropagation(); setFilePreview(tmpl) }}
                      title="View file"
                    >
                  ...
                  {count > 0 && (
                    <span className="template-card-count" style={{ marginTop: 6 }}>{resourceSummary(tmpl)}</span>
                  )}
                </div>
              </div>
            )
      ...
      <div className={`marketplace-footer ${hasChanges ? 'visible' : ''}`}>
        ...
        <button
          ...
          disabled={installState === 'installing' || !hasChanges}
          onClick={install}
        >
          {installState === 'installing' ? t.marketplace_saving
            : installState === 'installed' ? t.marketplace_saved
